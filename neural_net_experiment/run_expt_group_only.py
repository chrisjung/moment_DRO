...
def main():
    ...
    parser.add_argument('--only_group', default=-1, type=int) # -1 indicates train on all group
    # Resume?
    ...
    if args.only_group >= 0:
        indices_belong_to_this_group = torch.where((train_data._group_array == args.only_group))[0]
        train_data = DRODataset(Subset(train_data, indices_belong_to_this_group), process_item_fn=train_data.process_item, n_groups=train_data.n_groups,
