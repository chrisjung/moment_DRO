...
class CelebADataset(ConfounderDataset):
    ...
    # Inject noise group 0 and group 1's label
    # with probability flip_prob, it'll be a fair coin toss. OW, it retains its original label
    def flip_labels(self, flip_prob):
        group_1_indices = np.where(self.group_array == 1)[0]
        group_0_indices = np.where(self.group_array == 0)[0]

        group_indices = np.concatenate((group_0_indices, group_1_indices))
        num_data_points_to_be_flipped = int(len(group_indices) * flip_prob)

        if num_data_points_to_be_flipped > 0:
            indices_to_be_flipped = np.random.choice(group_indices, num_data_points_to_be_flipped, replace=False)
            new_labels = torch.randint(0, 2, size=(1, num_data_points_to_be_flipped))[0]

            self.y_array[indices_to_be_flipped] = new_labels
