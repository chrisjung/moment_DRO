...
from train_centered import train
...
def main():
    ...
    parser.add_argument('--group_min_loss', type=str, default=None)
    # Resume?
    ...
    group_min_losses = np.array([float(min_loss) for min_loss in args.group_min_loss.strip().split(',')])

    train(group_min_losses, model, criterion, data, logger, train_csv_logger, val_csv_logger, test_csv_logger, args, epoch_offset=epoch_offset)
